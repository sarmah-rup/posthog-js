...
describe('package.json#exports routing', () => {
    it('./pages routes each runtime to the correct barrel', () => {
        ...
        // Pages Router server bundles resolve the `node` condition (not
        // `react-server`), so it must route to the server barrel — with only
        // `default`, getServerSideProps code silently got the client barrel
        // where the server API is undefined.
...
describeIfBuilt('Transitive import closure of each built barrel', () => {
    ...
    // `pages.js` must NOT reach server-only: it is served to Pages Router
    // server bundles via the `node` condition, where server-only's
    // non-react-server build throws at import time. `index.js` may keep it
    // (only served under `react-server`, where server-only resolves empty).
    it.each([
